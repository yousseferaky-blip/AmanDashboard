...
const filteredMenuItems = menuItems.filter((item) => {
  ...
  return (
    <motion.div
      ...
      className={`
        fixed top-0 right-0 h-full bg-black shadow-2xl z-50
    ...
    >
      ...
      <div className="flex-shrink-0 p-2 sm:p-4 border-b border-gray-700 bg-black">
        <div className="flex items-center justify-between">
          {/* Logo Section */}
          <div className="flex items-center justify-center w-full">
            <button
              onDoubleClick={onToggleCollapse}
              className="flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-white rounded-lg sm:rounded-xl overflow-hidden shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-800"
              aria-label="شعار"
          ...
          <button
            ...
            className="flex items-center justify-center p-1 sm:p-2 rounded-lg hover:bg-gray-800 transition-colors"
              ...
              {item.hasDropdown ? (
                <div>
                  <motion.button
                    ...
                    className={`w-full flex items-center justify-between gap-2 sm:gap-3 px-2 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl transition-all duration-200 ${
                      isUsersActive()
                        ? "bg-white text-indigo-800 border-r-2 border-indigo-800 shadow-lg"
                        : "text-gray-300 hover:bg-white hover:text-indigo-800 hover:shadow-md"
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-3">
                      <item.icon
                        className={`w-4 h-4 sm:w-5 sm:h-5 ${
                          isUsersActive() ? "text-indigo-800" : "text-gray-400"
                        }`}
                      />
                      <AnimatePresence>
                        {!isCollapsed && (
                    ...
                    {!isCollapsed && (
                      ...
                      >
                        {usersDropdownOpen ? (
                          <HiChevronUp className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                        ) : (
                          <HiChevronDown className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                        )}
                      </motion.div>
                    )}
                  </motion.button>
                            ...
                            <Link
                              ...
                              className={`flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-1.5 sm:py-2 rounded-md sm:rounded-lg transition-all duration-200 text-xs sm:text-sm ${
                                isActive(subItem.path)
                                  ...
                                  : "text-gray-400 hover:bg-white hover:text-indigo-800"
                              }`}
                            >
                              <div
                  ...
                  {/* Collapsed Dropdown Items (icons only) */}
                  <AnimatePresence>
                    {usersDropdownOpen && isCollapsed && (
                      <motion.div
                        initial={{ opacity: 0, height: 0 }}
                  ...
                  {/* Tooltip for collapsed main item */}
                  {isCollapsed && (
                ...
                >
                  ...
                  >
                    <item.icon
                      className={`w-4 h-4 sm:w-5 sm:h-5 ${
                        isActive(item.path) ? "text-indigo-800" : "text-white"
                      }`}
                    />
                    <AnimatePresence>
                      {!isCollapsed && (
                  ...
                  {/* Tooltip for collapsed simple item */}
                  {isCollapsed && (
                    <div className="pointer-events-none absolute top-1/2 -translate-y-1/2 right-full mr-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                      <div className="bg-gray-900 text-white text-xs px-2 py-1 rounded-md shadow-lg whitespace-nowrap border border-gray-700">
                        {item.name}
      ...
      <div className="flex-shrink-0 p-2 sm:p-4 border-t border-gray-700 bg-gray-900">
        <motion.div
        ...
        >
          ...
          <div className="relative">
            <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br from-indigo-800 to-indigo-700 rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg">
              <HiUser className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-black" />
            </div>
            <div className="absolute -bottom-0.5 -right-0.5 sm:-bottom-1 sm:-right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 bg-green-500 rounded-full border border-gray-900 sm:border-2"></div>
          </div>
            ...
            {!isCollapsed && (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  >
                    <HiLogout className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                  </button>
                </div>
              </motion.div>
            )}
